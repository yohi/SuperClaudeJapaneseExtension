...
SuperClaude Framework のコマンドファイル（`~/.claude/commands/`）の説明を日本語化し、オプション説明を追加できます。
...
このコマンドは以下を実行します：
- `/sc` コマンド（24個）の説明を日本語化
- `/kiro` コマンド（10個）の説明を日本語化
- 各コマンド固有のオプション説明を追加（「オプション」セクション）
- 全コマンドで使用可能な共通フラグ説明を追加（「共通オプション」セクション）
- Claude Code 起動時に表示されるコマンド一覧が日本語で表示されるようになります

#### 追加される内容の例

コマンドファイルには以下のようなセクションが追加されます：
...
これにより、各コマンドで利用可能なオプションとその説明が日本語で確認できるようになります。
...
1. `translations/ja/commands.json` を編集
   - コマンド説明を追加
   - 必要に応じてコマンド固有のオプション説明を追加
2. 共通フラグを追加する場合は `translations/ja/flags.json` を編集
3. `npm run apply` を実行して変更を適用
4. テストを実行して確認
5. プルリクエストを作成
